...
allrea$zip <- formatC(allrea$zip, width=5, format="d", flag="0")
row.names(allrea) <- allrea$system
...
cleantable <- allrea %>%
  select(
    ...
    kWhPerResidentialCustomerDecember = op_kwh_pr,
...
set.seed(100)
readata <- allrea[sample.int(nrow(allrea), 10000),]
